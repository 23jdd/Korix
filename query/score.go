...
func scorePostings(searcher Searcher, field, term string, postings []inverted.Posting) (map[string]float64, error) {
	...
	for _, posting := range postings {
		...
		lengths, err := searcher.QueryMetadata(posting.DocID)
		if err != nil {
			return nil, err
		}
